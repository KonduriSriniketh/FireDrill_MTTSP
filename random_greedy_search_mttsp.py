...
class Greedy_Search_Method():
    ...
    def __init__(self,point_profile):
        ...
        self.final_output_sequence = np.empty((0,1))
        ...
        out = self.estimate_route_length(self.n_waypoints, ini_pos, ini_vel, ini_yaw, robot_pt, robot_vel_abs)
        print("self.final_output_sequence = ",self.final_output_sequence.astype(int))
        print("self.final_distance = ", self.final_distance)
        print("-------------------------------------")


    def estimate_route_length(self,n_waypoints, ini_pos, ini_vel, ini_yaw, robot_pt, robot_vel_abs):
        #initializations
        dt = 0
        ds = 0
        time = 0
        ...
        counter = np.arange(0,n_waypoints)
        a = random.randint(0, n_waypoints - 1)
        dt_min = 1000
        ds_min = 1000
        target_pt = ini_pos[a,:]
        target_vel = ini_vel[a]
        target_yaw = ini_yaw[a]
        updated_pt = self.update_pt_fun(target_pt, target_vel, target_yaw, time, robot_pt)
        intersection_pt = self.estimate_intersection(updated_pt, robot_pt, robot_vel_abs, target_vel, target_yaw)
        ...
        if (ds < ds_min):
            ...
            dt_min = dt
            min_index = a
            intersection_pt_min = intersection_pt
        robot_pt = intersection_pt_min
        counter = np.delete(counter, np.where(counter == a)[0])
        time = time + dt_min
        Distance = Distance + ds_min
        self.final_output_sequence = np.append(self.final_output_sequence, min_index)

        for f in range(1,n_waypoints):
            for h in counter:
                target_pt = ini_pos[h,:]
                target_vel = ini_vel[h]
                target_yaw = ini_yaw[h]
                updated_pt = self.update_pt_fun(target_pt, target_vel, target_yaw, time, robot_pt)
                intersection_pt = self.estimate_intersection(updated_pt, robot_pt, robot_vel_abs, target_vel, target_yaw)
                ds = self.euclidean_distance(robot_pt[0], robot_pt[1], intersection_pt[0], intersection_pt[1])
                dt = ds/robot_vel_abs
                if (ds < ds_min):
                    ...
                    intersection_pt_min = intersection_pt
            robot_pt = intersection_pt_min
            counter = np.delete(counter, np.where(counter == min_index)[0])
            time = time + dt_min
            Distance = Distance + ds_min
            self.final_output_sequence = np.append(self.final_output_sequence, min_index)
            robot_intersection_pts = np.vstack((robot_intersection_pts, intersection_pt_min))
...
def main():
    ...
    problem = Greedy_Search_Method(point_profile=points)
    print("point_profile = \n", problem.point_profile)
    fig=None
    ax=None
    show=True
    label=True
    ...
    plt.plot([problem.final_robot_intersection_pt[problem.n_waypoints - 1,0],problem.staircase_pt[0]],
                [problem.final_robot_intersection_pt[problem.n_waypoints - 1,1],problem.staircase_pt[1]],
                color = 'gray')

    plt.annotate("STAIRCASE", (problem.staircase_pt[0], problem.staircase_pt[1]))
    ...
    for i, txt in enumerate(n):
        ...
        k = int(problem.final_output_sequence[i])
        ...
        plt.annotate(txt, (problem.point_profile[i,0], problem.point_profile[i,1]))
        ...
        plt.arrow(problem.point_profile[i,0],problem.point_profile[i,1],
                    ...
                    width=.08, facecolor='blue', edgecolor='none' )
